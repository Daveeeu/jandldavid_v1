...
// ─── Mock API services ────────────────────────────────────────────────────────
// POST /api/ai/project-assistant — replace sendMessage() body with real fetch
// POST /api/contact/project     — replace submit() body with real fetch
...
const aiProjectAssistant = {
  async sendMessage(
    ...
    project: FormData
  ): Promise<{ content: string; type?: MessageType; summaryData?: ProjectSummaryData }> {
    await _delay(1100 + Math.random() * 900);
    const userMsgs = messages.filter((m) => m.role === "user");
    const idx = userMsgs.length - 1;

    if (idx < MOCK_QUESTIONS.length) {
      return { content: MOCK_QUESTIONS[idx] };
    }

    const featureText = userMsgs[2]?.content ?? "";
    const features = featureText.length > 2
      ? featureText.split(/[,;]+/).map((s) => s.trim()).filter(Boolean)
      : ["–"];

    const summaryData: ProjectSummaryData = {
      goal: project.description || userMsgs[0]?.content || "–",
      audience: userMsgs[1]?.content || "–",
      features,
      existingSystem: project.existingSystem
        ? `Igen${project.existingSystemUrl ? ` — ${project.existingSystemUrl}` : ""}`
        : "Nem",
      technical: "Webalapú platform, mobilbarát felület",
      integrations: "Nincs meghatározva",
      deadline: userMsgs[3]?.content || "Rugalmas",
      notes: project.consultation ? "Online konzultáció kért" : "",
    };
...
const projectContact = {
  async submit(payload: ContactPayload): Promise<void> {
    await _delay(1400);
    // Replace with:
    // await fetch("/api/contact/project", {
    //   method: "POST",
...
function ProjectForm({
  ...
  onFieldFocus, onFieldBlur, onSubmit, submitting, chatPhase,
}: {
  form: FormData;
...
export function ContactSection() {
  ...
  const sendToAI = useCallback(async (userText: string, currentMsgs: ChatMessage[]) => {
    ...
    try {
      const resp = await aiProjectAssistant.sendMessage(withUser, form);
      ...
      setChatError("Most nem sikerült választ kérni az asszisztenstől. Próbáld újra néhány másodperc múlva.");
    } finally {
  ...
  const retryLast = useCallback(async () => {
    ...
    fireEvent("ai_message_sent");
    try {
      const resp = await aiProjectAssistant.sendMessage(messages, form);
      const aiMsg: ChatMessage = { id: nextId(), role: "assistant", content: resp.content, timestamp: Date.now(), status: "sent", type: resp.type ?? "message", summaryData: resp.summaryData };
